#!/usr/bin/env bash
# GB10 tensor-core MMA calibration, native-only spark1. No remote Python/Rust.
set -euo pipefail
ROOT="${1:?}"; SOURCE="${2:?}"
[[ "$ROOT" =~ ^/var/tmp/mimo26f-kernel/calib-[[:alnum:]]{8}$ && "$SOURCE" =~ ^[0-9a-f]{40}$ ]] || exit 2
...
printf 'time=%s source=%s host=%s mode=tensor-calib\n' "$(date --iso-8601=seconds)" "$SOURCE" "$(hostname)"
...
printf 'CALIB FLAGS ';printf '%q ' "${flags[@]}";printf '\n'
"$NVCC" "${flags[@]}" -Xptxas=-v "$ROOT/tensor_calib.cu" -o "$ROOT/tensor-calib" 2>&1 | tee "$ROOT/receipts/compile.log"
sha256sum "$ROOT/tensor-calib" | tee "$ROOT/receipts/binaries-sha256.txt"
...
SAMPLE_PID=$!
set +e
timeout 300s "$ROOT/tensor-calib" | tee "$ROOT/receipts/calib.log"
codes=("${PIPESTATUS[@]}")
set -e
...
printf 'CALIB BATCH COMPLETE time=%s no_promotion=yes\n' "$(date --iso-8601=seconds)"
